...
def run(data_path, learner_type, fraction):
    ...
    if learner_type is "decision_tree":
        ltype = DecisionTree(attribute_names)
    elif learner_type is "prior_probability":
        ltype = PriorProbability()
    train_features, train_targets, test_features, test_targets = train_test_split(features, targets, fraction)
    ltype.fit(train_features, train_targets)
    pred = ltype.predict(test_features)
    conf_matrix = confusion_matrix(test_targets, pred)
    acc = accuracy(test_targets, pred)
    precision, recall = precision_and_recall(test_targets, pred)
    f1 = f1_measure(test_targets, pred)
